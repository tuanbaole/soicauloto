...
<?php if (!empty($content)) : ?>
	<?php $connectTime = 0; $connectCount = 0; ?>
	...
	<div class="sql-log-panel-query-log">
		...
		<?php
			...
			if (empty($queryLog['queries'])):
				...
				$connectTime = $connectTime + $queryLog['time'];
				$connectCount = $connectCount + $queryLog['count'];
				echo '<h5>';
				echo __d(
					'debug_kit',
	...
	<?php 
		$logSqlToFile = "Total Time:". $connectTime.  "ms. Total Queries:".$connectCount. " queries"; 
		//$this->log($logSqlToFile, 'sql_log'); 
	?>
<?php else:
